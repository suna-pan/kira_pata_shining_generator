...
(function() {
  $(function() {
    var draw_res, f_delta_h, f_delta_x, f_delta_y, f_rep_cnt, f_right, f_scale, file, hsv_to_rgb, proc_image, res_canvas, res_ctx, result, rgb_to_hsv, tmp_canvas, tmp_ctx;
    file = null;
    result = $('#result_image');
    f_rep_cnt = $('#rep_count');
    f_delta_x = $('#delta_x');
    ...
    tmp_canvas.hide();
    res_canvas.hide();
    draw_res = function(x, y, w, h, image_data, s) {
      var idx1, idx2, j, k, ref, ref1, res_image, xx, xxx, yy, yyy;
      res_image = res_ctx.getImageData(x, y, w, h);
      for (yy = j = 0, ref = h; 0 <= ref ? j <= ref : j >= ref; yy = 0 <= ref ? ++j : --j) {
        for (xx = k = 0, ref1 = w; 0 <= ref1 ? k <= ref1 : k >= ref1; xx = 0 <= ref1 ? ++k : --k) {
          xxx = Math.round(xx * s);
          yyy = Math.round(yy * s);
          idx1 = (w * yy + xx) * 4;
          idx2 = (w * yyy + xxx) * 4;
          if (image_data.data[idx1 + 3] !== 255) {
            continue;
          }
      ...
      return res_ctx.putImageData(res_image, x, y);
    };
    ...
    return $('#process').click(function() {
      var delta_h, delta_x, delta_y, image, reader, rep_cnt, right, scale;
      if (file === null) {
        return;
      }
      ...
      reader.onload = function(evt) {
        image.onload = function() {
          var dh, draw_x, draw_y, h, i, image_data, img, j, k, png, ref, ref1, rh, rw, s, w;
          res_canvas.hide();
          w = this.width;
          h = this.height;
          rw = w * Math.pow(scale, rep_cnt - 1);
          for (i = j = 1, ref = rep_cnt - 1; 1 <= ref ? j <= ref : j >= ref; i = 1 <= ref ? ++j : --j) {
            s = Math.pow(scale, i);
          ...
          draw_x = 0;
          if (right) {
            draw_x = rw - w * Math.pow(scale, rep_cnt - 1);
          }
          draw_y = 0;
          if (delta_y < 0) {
            draw_y = (rep_cnt - 1) * Math.abs(delta_y);
          }
          for (i = k = 0, ref1 = rep_cnt - 1; 0 <= ref1 ? k < ref1 : k > ref1; i = 0 <= ref1 ? ++k : --k) {
            ...
            if (right) {
              draw_x += (w * s - delta_x * s) - w * Math.pow(scale, dh - 1);
            }
            draw_y += delta_y;
          }
          image_data = tmp_ctx.getImageData(0, 0, w, h);
